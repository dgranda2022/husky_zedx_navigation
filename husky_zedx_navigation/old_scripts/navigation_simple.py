...
class WaypointNavigatorPD(Node):
    def __init__(self):
        ...
        # FIX: Explicitly defined all parameters as floats
        self.goal_xy_tolerance = 0.1
        self.goal_final_theta_tolerance = 0.1
        ...
        # FIX: Explicitly defined all error terms as floats
    ...
    def navigation_control_loop(self):
        ...
        if self.current_nav_state == self.NAV_STATE_ALIGNING_TO_POINT:
            if abs(heading_error_to_point) > self.approach_heading_tolerance:
                ...
                clamped_angular_velocity = max(min(angular_velocity, self.max_angular_speed), -self.max_angular_speed)
                twist_cmd.angular.z = float(clamped_angular_velocity) # FIX: Clamp first, then cast to float
            else:
                self.current_nav_state = self.NAV_STATE_MOVING_TO_POINT
                self.previous_distance_error = distance_to_goal_xy
                self.log_and_publish_status(f"Wpt {self.current_waypoint_index}: Alignment to point (x,y) complete.")
            ...
            if distance_to_goal_xy > self.goal_xy_tolerance:
                ...
                clamped_linear_velocity = max(min(linear_velocity, self.max_linear_speed), 0.0)
                twist_cmd.linear.x = float(clamped_linear_velocity) # FIX: Clamp first, then cast to float

                angular_error_derivative = (heading_error_to_point - self.previous_heading_error_to_point) / dt
                angular_velocity = (self.angular_Kp * heading_error_to_point) + (self.angular_Kd * angular_error_derivative)
                clamped_angular_velocity = max(min(angular_velocity, self.max_angular_speed * 0.7), -self.max_angular_speed * 0.7)
                twist_cmd.angular.z = float(clamped_angular_velocity) # FIX: Clamp first, then cast to float
            else:
            ...
            if self.current_waypoint_index < len(self.target_waypoints):
                ...
                self.previous_heading_error_to_point = 0.0
                self.previous_distance_error = 0.0
                self.previous_final_heading_error = 0.0 # FIX: Reset all previous errors
...
def main(args=None):
    ...
    try:
        ...
        # FIX: Check if context is still valid before shutdown
        if rclpy.ok():
